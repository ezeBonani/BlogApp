...
export default function DashProfile() {
  ...
  const handleImageChange = (e) => {
    ...
    if (file) {
      setImageFile(file);
      setImageFileUrl(URL.createObjectURL(file)); //creo una imagen url a partir del archivo subido para mostrar temporalmente
    }
  };
  //useEffect para detectar nueva imagen
  useEffect(() => {
    if (imageFile) {
      uploadImage();
    }
    ...
    uploadTask.on(
      ...
      (error) => {
        setImageFileUploadError(
          "Could not upload file, it must be an image and less than 2 MB"
        );
